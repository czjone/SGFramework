	...
	namespace Security {
		...
		public class DesEncryption : IEncryption {
			...
			public bool checkKey (byte[] key) {
				CheckKeyArgumentWithException (key);
				return true;
			}
			...
			/// 检查key
			/// </summary>
			/// <param name="key">目录key</param>
			/// <returns>返回key是否合法</returns>
			public bool VerificationKey (byte[] key) {
				if (key == null || key.Length != keyIvMinCount) {
					return false;
				}
				return true;
			}
			...
			/// </summary>
			/// <param name="key"></param>
			public static void CheckKeyArgumentWithException (byte[] key) {
