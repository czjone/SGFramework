namespace SGF.Unity {
	...
	public class FileUtils : System.IDisposable {
		...
		public static FileUtils GetInstance () {
			if (utils == null) utils = new FileUtils ();
			return utils;
		}
		...
		public byte[] GetStreamAssetsBytes (string fname) {
			...
			//if android platform read stream form pkg with www.
			//other read stream from files.
		...
		public UnityEngine.AssetBundle GetAssetsBundleWithBytes (byte[] buf) {
			UnityEngine.AssetBundle ab;
			using (var stream = new System.IO.MemoryStream (buf)) {
				ab = UnityEngine.AssetBundle.LoadFromStream (stream);
				stream.Close ();
			}
			return ab;
		}

		public UnityEngine.AssetBundle GetAssetsBundleFromStreamAssets (string fname) {
			byte[] buf = GetStreamAssetsBytes (fname);
			return GetAssetsBundleWithBytes (buf);
		}

		public UnityEngine.AssetBundle GetAssetsBundleFromPersistentData (string fname) {
